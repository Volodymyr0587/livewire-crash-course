    ...
    <div class="flex">
        <div class="ml-3 mt-1 flex-1 md:flex md:justify-between">
            <p class="text-sm font-bold text-pink-700">
                <a href="{{ route('task-show', $task->id) }}" wire:navigate>{{ $task->name }}</a> / {{ Carbon\Carbon::parse($task->updated_at)->format('d-m-Y H:i') }}
            </p>
            <p class="mt-2 text-md md:ml-6 md:mt-0">
                ...
                <button wire:click="delete({{ $task->id }})" class="whitespace-nowrap font-medium text-pink-700 hover:text-pink-600">
